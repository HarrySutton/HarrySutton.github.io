 ...
 */
function invokeList(len, invoke){
	let list = [];
	for (let i = 0; i < len; i++){
		list.push(invoke(i))
	};
...
function invokeMatrix(w, h, invoke){
	let matrix = [];
	for (let x = 0; x < w; x++){
		let list = [];
		for (let y = 0; y < h; y++){
 ...
 * @returns {function} 	Function that calls given function with given parameters
 */
function useFunc(func, params){
	return () => func(...params)
}